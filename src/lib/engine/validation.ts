...
function getAllDates(data: MarketData): string[] {
  const refTicker = data.prices['SPY.US'] ? 'SPY.US' : data.prices['SPY'] ? 'SPY' : data.prices['XAUUSD.FOREX'] ? 'XAUUSD.FOREX' : data.prices['QQQ.US'] ? 'QQQ.US' : Object.keys(data.prices)[0];
